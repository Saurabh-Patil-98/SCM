...
const StudentTable = () => {
  ...
  const myResponse = async () => {
    try {
      ...
      const response = await AxiosAPI.get("/student/4F5455324D54453D" + url, {
      ...
      //   "http://localhost:2525/student/4F5455324D54453D",
      //   {
  ...
  // show single data
  const showDetail = async (student_id) => {
    try {
      // const url = localStorage.getItem("url");
      // const token = localStorage.getItem("token");
      // const response = await AxiosAPI.get("/student/edit/4F5455324D54453D/${student_id}` + url, {
      //   headers: { Authorization: "Bearer " + token },
      // });
      const accessToken =
        "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIxLGR1cmdlc2giLCJpc3MiOiJXYXRlciBTb2Z0d2FyZXMiLCJyb2xlcyI6IltST0xFX1VTRVJdIiwiaWF0IjoxNzE2NDM5NTQ3LCJleHAiOjE3MTY1MjU5NDd9.0LgLxrYtAqugeI9wLsiaoFgk2xbc76EuM2xbDDtKOkAntrelJkAocbQpbPkLnuale-U3Mvef8VqV5Z0lg8qHdw";
      const response = axios.get(
        `http://localhost:2525/student/edit/4F5455324D54453D/${student_id}`,
                            ...
                            {records
                              ...
                              .map((names, index) => (
                                <tr key={index}>
                                  ...
                                  <td>{names.student_name}</td>
                                  <td>{names.dob}</td>
                                  <td>{names.address}</td>
                                  <td>{names.email}</td>
                                  <td>{names.gender}</td>
                                  <td>{names.mobile}</td>
                                  <td>{names.previous_school}</td>
                                  <td
                                    data-toggle="tooltip"
                    ...
                    >
                      ...
                      <div className="relative h-full p-3 bg-white border-2 border-blue-500 rounded-lg">
                        ...
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Student Name : {modeldata.student_name}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                        ...
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Mobile Number : {modeldata.mobile}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Previous School : {modeldata.previous_school}
                          </h3>
                        </span>
                      </div>
                    </div>
