...
const StudentStandardTable = () => {
  ...
  const myResponse = async () => {
    try {
      ...
      const response = axios.get(
        "http://localhost:2525/studentStandard/4F5455324D54453D",
        {
  ...
  // show single data
  const showDetail = async (student_standard_id) => {
    try {
      const accessToken =
        "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIxLGR1cmdlc2giLCJpc3MiOiJXYXRlciBTb2Z0d2FyZXMiLCJyb2xlcyI6IltST0xFX1VTRVJdIiwiaWF0IjoxNzE2NTQ4OTM3LCJleHAiOjE3MTY2MzUzMzd9.dKzK8OZcKxqg7SN2IvW2oMceCcZzw2cTkzREWNzGKiOXX3WffBBSXb3s1w97bcsi2CrbgRzmAyrHy35iCv4APQ";
      const response = axios.get(
        `http://localhost:2525/studentStandard/edit/4F5455324D54453D/${student_standard_id}`,
                            ...
                            {records
                              .filter((item) => {
                                return search.toLowerCase() === ""
                                  ? item
                                  : item.academic_end_year
                                      .toLowerCase()
                                      .includes(search);
                              })
                              .map((names, index) => (
                                <tr key={index}>
                                  <td>{names.student_standard_id}</td>
                                  <td>{names.academic_end_year}</td>
                                  <td>{names.academic_start_year}</td>
                                  <td>{names.admission_date}</td>
                                  <td>{names.grade}</td>
                                  <td
                                    data-toggle="tooltip"
                                  ...
                                  >
                                    <button
                                      ...
                                      onClick={(e) =>
                                        showDetail(names.student_standard_id)
                                      }
                                    >
                                      <FaEye />
                                    </button>
                      ...
                      <div className="relative h-full p-3 bg-white border-2 border-blue-500 rounded-lg">
                        ...
                        <div className="flex items-center -mt-1">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Row No : {modeldata.student_standard_id}
                          </h3>
                        </div>
                        ...
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Academic End Year : {modeldata.academic_end_year}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Academic Start Year: {modeldata.academic_start_year}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            Admission Date : {modeldata.admission_date}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
...
export default StudentStandardTable;
