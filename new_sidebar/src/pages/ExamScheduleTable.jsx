...
const ExamScheduleTable = () => {
  ...
  const myResponse = async () => {
    try {
      ...
      const response = axios.get(
        "http://localhost:2525/examschedule/4F5455324D54453D",
        {
  ...
  // show single data
  const showDetail = async (exam_id) => {
    try {
      const accessToken =
        "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIxLGR1cmdlc2giLCJpc3MiOiJXYXRlciBTb2Z0d2FyZXMiLCJyb2xlcyI6IltST0xFX1VTRVJdIiwiaWF0IjoxNzE2NDM5NTQ3LCJleHAiOjE3MTY1MjU5NDd9.0LgLxrYtAqugeI9wLsiaoFgk2xbc76EuM2xbDDtKOkAntrelJkAocbQpbPkLnuale-U3Mvef8VqV5Z0lg8qHdw";
      const response = axios.get(
        `http://localhost:2525/examschedule/edit/4F5455324D54453D/${exam_id}`,
                          ...
                          <tbody>
                            {records
                              ...
                              .map((names, index) => (
                                <tr key={index}>
                                  <td>{names.exam_id}</td>
                                  <td>{names.exam_name}</td>
                                  <td>{names.exam_center}</td>
                                  <td>{names.exam_date}</td>
                                  <td>{names.exam_start_time}</td>
                                  <td>{names.exam_end_time}</td>
                                  <td>{names.status}</td>
                                  <td
                                    data-toggle="tooltip"
                    ...
                    >
                      ...
                      <div className="relative h-full p-3 bg-white border-2 border-blue-500 rounded-lg">
                        ...
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            exam_name : {modeldata.exam_name}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            exam_center : {modeldata.exam_center}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            exam_date : {modeldata.exam_date}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            exam_start_time : {modeldata.exam_start_time}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
                            exam_end_time : {modeldata.exam_end_time}
                          </h3>
                        </span>
                        <span className="my-2 text-gray-600">
                          <h3 className="my-2 ml-3 text-lg font-bold text-gray-800">
